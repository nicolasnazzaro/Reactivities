...
axios.interceptors.response.use(undefined, error => {
    ...
    if (status === 401 && headers['www-authenticate'].includes(
...
const Profiles = {
    ...
    deletPhoto: (id: string) => request.del(`/photos/${id}`),
    ...
    listActivites: (username: string, predicate: string) => request.get(`/profiles/${username}/activities?predicate=${predicate}`)
}
